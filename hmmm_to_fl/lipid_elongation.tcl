...
set InFile0 [open "./memb.pdb" r]
set OutFile [open "./memb2.pdb" w]

# Open a new temporary output file for writing the cleaned data
set NewInFile [open "./memb_cleaned.pdb" w]

# Read each line of the input file (using InFile0, which was opened for memb.pdb)
while {[gets $InFile0 line] >= 0} {
    
    # 1. Check if the line is an ATOM or HETATM record (PDB columns 1-6, indices 0-5)
    set recordType [string trim [string range $line 0 5]]

    if {($recordType == "ATOM") || ($recordType == "HETATM")} {

        # Ensure the line is long enough to access PDB fields
        if {[string length $line] < 22} {
            puts $NewInFile $line
            continue
        }

        # --- A. Neutralize Alternate Location Indicator (PDB Column 17, Index 16) ---
        set cleaned_line [string replace $line 16 16 " "]

        # --- B. Neutralize Chain Identifier (PDB Column 22, Index 21) ---
        set cleaned_line [string replace $cleaned_line 21 21 " "]

        # --- C. Write the cleaned line ---
        puts $NewInFile $cleaned_line

    } else {
        # Keep all other lines (CRYST1, END, etc.) as is to maintain file structure
        puts $NewInFile $line
    }
}
...
#######################################################################################
####           Making an standard or cleaned pdb out of the input pdb              ####
#######################################################################################
set InFile [open "./memb_cleaned.pdb" r]
...
while {[gets $InFile line] >= 0} {
    ...
    # CRITICAL FIX: Check if the line has at least 8 fields 
    # (needed for ResName, AtomName, and Coordinates)
    # The minimum is 8 elements (0 to 7) to safely access coordinates (index 7 for zValue)
    set line_list [split $line]
    if {[llength $line_list] < 8} {
        continue
    }


# Locate to C26 atom
    if {[lindex $line 3] != "CHL1" && [lindex $line 2] == "C26"} {
    	

	# Exclude any resname that starts or ends with PI
    	set resname [lindex $line 3]
    	if {[string match "PI*" $resname] || [string match "*PI" $resname]} {
        	continue
    	}
	...
	# Keep C26 atom in the output pdb
      ...
      # Locate to C36 atom
    } elseif {[lindex $line 3] != "CHL1" && [lindex $line 2] == "C36"} {
	...
	# Locate to C6S atom
    } elseif {[lindex $line 3] != "CHL1" && [lindex $line 2] == "C6S"} {

	# Keep C6S atom in the output pdb
        set newLine $line
        lset newLine 2 C6S
        writePDBformat $newLine $OutFile
        incr atmNum

        # Get the coordinates of C6S atom
        set xValue [lindex $line 5]
        set yValue [lindex $line 6]
        set zValue [lindex $line 7]
	...
	# Add H6U atom
        ...
        # The H6T atom in the truncated HMMM lipid is ignnored
        # Add C7S and C8S atoms with adjusted z-coordinates referring to the z-position of C6S
        gets $InFile line
        # If lipid is in the upper leaflet, put C7S and C8S atoms below C6S atom; otherwise, put them above C6S atom
        set c7szValue [expr {$zValue > 0 ? $zValue - 1.5 : $zValue + 1.5}]
        set c8szValue [expr {$zValue > 0 ? $zValue - 3.0 : $zValue + 3.0}]

        lset newLine 1 $atmNum
        lset newLine 7 $c7szValue
        lset newLine 2 C7S
        writePDBformat $newLine $OutFile
        incr atmNum

        lset newLine 1 $atmNum
        lset newLine 7 $c8szValue
        lset newLine 2 C8S
        writePDBformat $newLine $OutFile

        # Locate to C6F atom
    } elseif {[lindex $line 3] != "CHL1" && [lindex $line 2] == "C6F"} {
        # Keep C6F atom in the output pdb
        set newLine $line
        lset newLine 2 C6F
        writePDBformat $newLine $OutFile
        incr atmNum

        # Get the coordinates of C6F atom
        set xValue [lindex $line 5]
        set yValue [lindex $line 6]
        set zValue [lindex $line 7]
	...
	# Add H6G atom
        ...
        # The H6H atom in the truncated HMMM lipid is ignnored
        # Add C7F and C8F atoms with adjusted z-coordinates referring to the z-position of C6F
        gets $InFile line
        # If lipid is in the upper leaflet, put C7F and C8F atoms below C6F atom; otherwise, put them above C6F atom
        set c7fzValue [expr {$zValue > 0 ? $zValue - 1.5 : $zValue + 1.5}]
        set c8fzValue [expr {$zValue > 0 ? $zValue - 3.0 : $zValue + 3.0}]

        lset newLine 1 $atmNum
        lset newLine 7 $c7fzValue
        lset newLine 2 C7F
        writePDBformat $newLine $OutFile
        incr atmNum

        lset newLine 1 $atmNum
        lset newLine 7 $c8fzValue
        lset newLine 2 C8F
        writePDBformat $newLine $OutFile
...
close $OutFile
close $InFile
puts "The addition of C27, C28, C37, C38, C7F, C8F, C7S, and C8S atoms is complete."
...
set requiredtopfiles {../toppar/top_all36_prot.rtf ../toppar/top_all36_lipid.rtf ../toppar/top_all36_carb.rtf ../toppar/top_all36_cgenff.rtf ../toppar/toppar_water_ions_namd.str ../toppar/toppar_all36_lipid_cholesterol.str ../toppar/toppar_all36_lipid_sphingo.str ../toppar/toppar_all36_lipid_inositol.str ../toppar/toppar_all36_carb_glycolipid.str}
set extratopfiles [open "./extratopfiles.dat" w]; # Extra topology files that are needed but weren't listed in the requiredtopfiles
